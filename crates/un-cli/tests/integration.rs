...
fn test_sync_file_repo() {
    ...
    let url = format!(
        "file://{}",
        source_dir.canonicalize().unwrap().to_string_lossy()
    );

    // Simulate sync
    let cache = un_cache::Cache::with_custom_root(
        std::env::current_dir()
            .unwrap()
            .join("tests/test_data/cache"),
    );
    let remote = un_git::GitRemote::new(&url);
    let database = un_git::GitDatabase::new(&cache, "test", &url).unwrap();
    let oid = database
        .fetch(&remote, &un_core::GitReference::DefaultBranch, false, false)
        .unwrap();

    // Create workspace
    let workspace_dir = std::env::current_dir()
        .unwrap()
        .join("tests/test_data/workspace");
    std::fs::create_dir_all(&workspace_dir).unwrap();

    // Checkout
    let _checkout = un_git::GitCheckout::new(
        &database,
        &oid,
        &workspace_dir.join("repo"),
        un_git::CheckoutMode::Worktree,
    )
    .unwrap();
